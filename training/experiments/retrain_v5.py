...
MERGED_CSV  = os.path.join(WORK_DIR, "augmented_pos700_dataset.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v5_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v5.csv")
...
V3_METRICS = {
    "accuracy": 0.7574, "macro_f1": 0.7164, "weighted_f1": 0.7551,
    "POSITIVE": 0.6552, "NEUTRAL": 0.7190,
    "CONSTRUCTIVE": 0.8221, "TOXIC": 0.6692,
}
V4_METRICS = {
    "accuracy": 0.7253, "macro_f1": 0.7024, "weighted_f1": 0.7288,
    "POSITIVE": 0.6316, "NEUTRAL": 0.7109,
    "CONSTRUCTIVE": 0.7596, "TOXIC": 0.7076,
}
...
print(f"Rows: total={len(df)} "
      ...
      f"test={int((df.split=='test').sum())}  aug={int(aug_mask.sum())}")
print("✅ [Step 2.1 — merged data loaded]")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v5, pos700 aug)")
print("═"*64)
...
print("\n── Comparison: v1 | v3 | v4 | v5 ──")
print(f"| {'Metric':<15s} | {'v1':>7s} | {'v3':>7s} | {'v4':>7s} | {'v5':>7s} | {'Δ v1→v5':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v1 = V1_METRICS[key]; v3 = V3_METRICS[key]; v4 = V4_METRICS[key]; v5 = new_metrics[key]
    print(f"| {label:<15s} | {v1:>7.4f} | {v3:>7.4f} | {v4:>7.4f} | {v5:>7.4f} | {pct(v1, v5):>8s} |")
