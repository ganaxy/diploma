...
MERGED_CSV  = os.path.join(WORK_DIR, "relabeled_v7_aug_v2_normalized.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v7_aug_v2_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v7_aug_v2.csv")
...
print("✅ [Step 2.1 — v7+aug_v2 data loaded]")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v7 + aug_v2)")
print("═"*64)
...
print("\n── Comparison: v7 | v7+aug_v2 ──")
print(f"| {'Metric':<15s} | {'v7':>7s} | {'v7+aug':>7s} | {'Δ':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    print(f"| {label:<15s} | {v7:>7.4f} | {nv:>7.4f} | {pct(v7, nv):>8s} |")
