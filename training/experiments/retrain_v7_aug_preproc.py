...
MERGED_CSV  = os.path.join(WORK_DIR, "relabeled_v7_aug_v2_preproc_normalized.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v7_aug_v2_preproc_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v7_aug_v2_preproc.csv")
...
EXTRA_SPECIAL_TOKENS = ["[EMOJI]", "[URL]", "[NUM]", "[HTML]"]
...
V7_AUG_METRICS = {
    "accuracy": 0.7861, "macro_f1": 0.7482, "weighted_f1": 0.7867,
    "POSITIVE": 0.6311, "NEUTRAL": 0.7573,
    "CONSTRUCTIVE": 0.8366, "TOXIC": 0.7676,
}
...
n_aug = int((df["source"].astype(str) == "augmented").sum())
print(f"Rows: total={len(df)} "
      f"train={int((df.split=='train').sum())} "
      f"val={int((df.split=='val').sum())} "
      f"test={int((df.split=='test').sum())}  aug={n_aug}")
print("✅ [Step 2.1 — v7+aug_v2+preproc data loaded]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
n_added = tokenizer.add_special_tokens(
    {"additional_special_tokens": EXTRA_SPECIAL_TOKENS}
)
print(f"✅ [Step 2.2 — tokenizer loaded; added {n_added} special tokens "
      f"({EXTRA_SPECIAL_TOKENS}); vocab now {len(tokenizer)}]")
for t in EXTRA_SPECIAL_TOKENS:
    ids = tokenizer.encode(t, add_special_tokens=False)
    print(f"   {t} → ids {ids}  (should be a single id)")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v7 + aug_v2 + doc-preproc)")
print("═"*64)
...
print("\n── Comparison: v7 | v7+aug_v2 | v7+aug_v2+preproc ──")
print(f"| {'Metric':<15s} | {'v7':>7s} | {'+aug':>7s} | {'+preproc':>9s} | "
      f"{'Δ vs v7':>9s} | {'Δ vs +aug':>10s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*11}|{'-'*11}|{'-'*12}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v7 = V7_METRICS[key]; va = V7_AUG_METRICS[key]; nv = new_metrics[key]
    print(f"| {label:<15s} | {v7:>7.4f} | {va:>7.4f} | {nv:>9.4f} | "
          f"{pct(v7, nv):>9s} | {pct(va, nv):>10s} |")
...
print("\n✅ STEP 2 (v7 + aug_v2 + doc-preproc) complete.")
