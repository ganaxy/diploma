...
MERGED_CSV  = os.path.join(WORK_DIR, "relabeled_v7_preproc_normalized.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v7_preproc_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v7_preproc.csv")
...
print(f"Rows: total={len(df)} "
      ...
      f"test={int((df.split=='test').sum())}  aug={n_aug}  (expect aug=0)")
assert n_aug == 0, "v7+preproc-only must have zero augmented rows"
print("✅ [Step 2.1 — v7+preproc data loaded, aug=0]")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v7 + doc-preproc, NO aug)")
print("═"*64)
...
print("\n── Comparison: v7 | v7+preproc ──")
print(f"| {'Metric':<15s} | {'v7':>7s} | {'+preproc':>9s} | {'Δ':>9s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*11}|{'-'*11}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v7 = V7_METRICS[key]; nv = new_metrics[key]
    print(f"| {label:<15s} | {v7:>7.4f} | {nv:>9.4f} | {pct(v7, nv):>9s} |")
...
print("\n✅ STEP 2 (v7 + doc-preproc, NO aug) complete.")
