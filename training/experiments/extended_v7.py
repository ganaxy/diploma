...
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v7_extended_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v7_extended.csv")
...
BASELINE_TAG = "v7 (20ep/p5)"
NEW_TAG      = "v7-extended (50ep/p10)"
...
if n_aug:
    print(f"WARN: found {n_aug} rows with source=='augmented' (unexpected)")
print(f"Rows: total={len(df)} "
      f"train={int((df.split=='train').sum())} "
      f"val={int((df.split=='val').sum())} "
      f"test={int((df.split=='test').sum())}  aug={n_aug}")
print(f"Hyperparameters: lr={LR}, MAX_EPOCHS={MAX_EPOCHS}, patience={PATIENCE}")
print("✅ [Step A1.1 — v7 data loaded]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
print("✅ [Step A1.2 — tokenizer loaded]")
...
print("✅ [Step A1.4 — class weights computed]")
...
def do_full_training(batch_size):
    print(f"\nTraining with batch_size={batch_size}, max_length={MAX_LENGTH}, "
          f"lr={LR}, MAX_EPOCHS={MAX_EPOCHS}, patience={PATIENCE}")
...
print("\n" + "═"*64)
print(f" Run A — Test-set results ({NEW_TAG})")
print("═"*64)
...
for i, row in enumerate(cm):
    ...
    print(f"  {LABEL_NAMES[i]:<12s} {cells}")
...
print(f"\n── Comparison: {BASELINE_TAG} | {NEW_TAG} ──")
print(f"| {'Metric':<15s} | {BASELINE_TAG:<14s} | {NEW_TAG:<22s} | {'Δ':>8s} |")
print(f"|{'-'*17}|{'-'*16}|{'-'*24}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    b = BASELINE_METRICS[key]; n = new_metrics[key]
    print(f"| {label:<15s} | {b:>14.4f} | {n:>22.4f} | {pct(b, n):>8s} |")
print(f"| {'Best epoch':<15s} | {BASELINE_METRICS['best_epoch']:>14d} | "
      f"{best_epoch:>22d} | {'—':>8s} |")
...
print(f"\n✅ [Step A1.6 — saved {len(out)} test predictions to {PRED_CSV}]")
print("\n✅ STEP A1 complete.  "
      "STOP — waiting for confirmation to run Run B (v9 extended).")
