...
MERGED_CSV  = os.path.join(WORK_DIR, "relabeled_v9_normalized.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v9_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v9.csv")
...
V7_METRICS = {
    "accuracy": 0.8064, "macro_f1": 0.7698, "weighted_f1": 0.8052,
    "POSITIVE": 0.6593, "NEUTRAL": 0.7697,
    "CONSTRUCTIVE": 0.8539, "TOXIC": 0.7964,
}
...
n_aug = int(aug_mask.sum())
if n_aug:
    print(f"WARN: found {n_aug} rows with source=='augmented' (unexpected for v9)")
print(f"Rows: total={len(df)} "
      f"train={int((df.split=='train').sum())} "
      f"val={int((df.split=='val').sum())} "
      f"test={int((df.split=='test').sum())}  aug={n_aug}")
print(f"Hyperparameters: lr={LR}, patience={PATIENCE} (identical to v7)")
print("✅ [Step 2.1 — v9 data loaded]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
print(f"✅ [Step 2.2 — tokenizer loaded]")

print("Computing p95 token length on train split ...")
...
print(f"Class weights (CE): {dict(zip(LABEL_NAMES, [round(w,3) for w in class_weights.tolist()]))}")
sample_weights = np.array([class_weights[y] for y in train_labels_np], dtype=np.float64)
print(f"✅ [Step 2.5 — class weights recomputed]")
...
def is_oom(err: BaseException) -> bool:
    s = str(err).lower()
    return any(k in s for k in ["out of memory","oom","allocation","alloc failed","cuda error: out","directml","hipout"])
...
def do_full_training(batch_size):
    print(f"\nTraining with batch_size={batch_size}, max_length={MAX_LENGTH}, lr={LR}, patience={PATIENCE}")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v9, latest relabel, no aug)")
print("═"*64)
...
print("\n── Comparison: v7 | v9 ──")
print("NOTE: 44 test labels changed v7→v9; Δ reflects both model-change "
      "and test-label-change.")
print(f"| {'Metric':<15s} | {'v7':>7s} | {'v9':>7s} | {'Δ v7→v9':>8s} | {'Verdict':<7s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*10}|{'-'*9}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v7 = V7_METRICS[key]; v9 = new_metrics[key]
    d = v9 - v7
    print(f"| {label:<15s} | {v7:>7.4f} | {v9:>7.4f} | {pct(v7, v9):>8s} | {verdict(d):<7s} |")

macro_delta_pct = (macro_f1 - V7_METRICS["macro_f1"]) * 100
print("\n── Verdict ─────────────────────────────────────────")
if macro_delta_pct > 0.3:
    print(f"  Macro F1 improved by {macro_delta_pct:+.2f} pct-pts vs v7.")
    print("  Relabeling helped — continue to next round.")
elif macro_delta_pct < -0.3:
    print(f"  Macro F1 dropped by {macro_delta_pct:+.2f} pct-pts vs v7.")
    print("  STOP SIGNAL — relabeling has reached its ceiling.")
else:
    print(f"  Macro F1 moved {macro_delta_pct:+.2f} pct-pts vs v7 "
          f"(within ±0.3 pct-pt band).")
    print("  Marginal — borderline, user decision needed.")

test_df = df[df["split"] == "test"].copy().reset_index(drop=True)
assert len(test_df) == len(y_pred), "test row mismatch"
out = pd.DataFrame({
