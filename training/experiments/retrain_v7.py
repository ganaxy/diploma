...
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v7_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v7.csv")
...
V6_METRICS = {
    "accuracy": 0.7907, "macro_f1": 0.7652, "weighted_f1": 0.7921,
    "POSITIVE": 0.7150, "NEUTRAL": 0.7679,
    "CONSTRUCTIVE": 0.8353, "TOXIC": 0.7427,
}
...
if n_aug:
    print(f"WARN: found {n_aug} rows with source=='augmented' (unexpected for v7)")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v7, relabeled-twice, no aug)")
print("═"*64)
...
print("\n── Comparison: v6 | v7 ──")
print("NOTE: 83 test labels changed v6→v7; Δ reflects both model-change "
      "and test-label-change.")
print(f"| {'Metric':<15s} | {'v6':>7s} | {'v7':>7s} | {'Δ v6→v7':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v6 = V6_METRICS[key]; v7 = new_metrics[key]
    print(f"| {label:<15s} | {v6:>7.4f} | {v7:>7.4f} | {pct(v6, v7):>8s} |")
