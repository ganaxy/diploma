...
MERGED_CSV  = os.path.join(WORK_DIR, "augmented_pos_only_dataset.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v4_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v4.csv")
...
if not os.path.exists(MERGED_CSV):
    print(f"ERROR: {MERGED_CSV} not found. Run step1_merge_pos_only.py first.")
    raise SystemExit(1)
...
print(f"Rows: total={len(df)} "
      ...
      f"aug={int(aug_mask.sum())}")
print("✅ [Step 2.1 — merged data loaded]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
print(f"✅ [Step 2.2 — tokenizer loaded]")

print("Computing p95 token length on train split (POSITIVE-aug included)...")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v4, POSITIVE-only aug)")
print("═"*64)
...
print("\n── Comparison: v1 best | v3 aug-all | v4 aug-pos-only ──")
print(f"| {'Metric':<15s} | {'v1':>7s} | {'v3':>7s} | {'v4':>7s} | {'Δ v1→v4':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v1 = V1_METRICS[key]; v3 = V3_METRICS[key]; v4 = new_metrics[key]
    print(f"| {label:<15s} | {v1:>7.4f} | {v3:>7.4f} | {v4:>7.4f} | {pct(v1, v4):>8s} |")

test_df = df[df["split"] == "test"].copy().reset_index(drop=True)
assert len(test_df) == len(y_pred), f"test row mismatch"
out = pd.DataFrame({
