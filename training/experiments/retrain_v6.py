...
MERGED_CSV  = os.path.join(WORK_DIR, "relabeled_normalized.csv")
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v6_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v6.csv")
...
if n_aug:
    print(f"WARN: found {n_aug} rows with source=='augmented' (unexpected for v6)")
print(f"Rows: total={len(df)} "
      f"train={int((df.split=='train').sum())} "
      f"val={int((df.split=='val').sum())} "
      f"test={int((df.split=='test').sum())}  aug={n_aug}")
print("✅ [Step 2.1 — relabeled data loaded]")
...
print("\n" + "═"*64)
print(" STEP 2 — Test-set results (MN-BERT v6, relabeled, no aug)")
print("═"*64)
...
print("\n── Comparison: v1 | v6 ──")
print(f"| {'Metric':<15s} | {'v1':>7s} | {'v6':>7s} | {'Δ v1→v6':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v1 = V1_METRICS[key]; v6 = new_metrics[key]
    print(f"| {label:<15s} | {v1:>7.4f} | {v6:>7.4f} | {pct(v1, v6):>8s} |")
