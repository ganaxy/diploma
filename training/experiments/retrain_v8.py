...
SAVE_DIR    = os.path.join(WORK_DIR, "best_mnbert_v8_model")
PRED_CSV    = os.path.join(WORK_DIR, "test_predictions_v8.csv")
...
if n_aug:
    print(f"WARN: found {n_aug} rows with source=='augmented' (unexpected for v8)")
print(f"Rows: total={len(df)} "
      f"train={int((df.split=='train').sum())} "
      f"val={int((df.split=='val').sum())} "
      f"test={int((df.split=='test').sum())}  aug={n_aug}")
print(f"Hyperparameters: lr={LR}, patience={PATIENCE} (v8 overrides)")
print("✅ [Step 2.1 — v7 data loaded]")
...
print("\n" + "═"*64)
print(" STEP 1 — Test-set results (MN-BERT v8, tighter HP, no aug)")
print("═"*64)
...
print("\n── Comparison: v7 | v8 ──")
print(f"| {'Metric':<15s} | {'v7':>7s} | {'v8':>7s} | {'Δ v7→v8':>8s} |")
print(f"|{'-'*17}|{'-'*9}|{'-'*9}|{'-'*10}|")
for key, label in [
    ("accuracy",     "Accuracy"),
    ("macro_f1",     "Macro F1"),
    ...
    v7 = V7_METRICS[key]; v8 = new_metrics[key]
    print(f"| {label:<15s} | {v7:>7.4f} | {v8:>7.4f} | {pct(v7, v8):>8s} |")
