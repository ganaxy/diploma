...
print(f"✅ [Step 1 — data loaded with text_normalized + source prepend]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
print(f"✅ [Step 2 — tokenizer loaded]")
...
for i in range(0, len(train_texts), BATCH):
    chunk = train_texts[i:i+BATCH]
    enc = tokenizer(chunk, add_special_tokens=True, truncation=False,
                    padding=False)
...
print(f"  train token lengths: p50={p50} p95={p95} p99={p99} max={p_max}")
...
print(f"✅ [Step 3 — max_length/batch_size decided]")
...
def do_full_training(batch_size):
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            tr_pred.append(preds.detach().cpu().numpy())

        tr_loss = float(np.mean(tr_losses))
        tr_true_c = np.concatenate(tr_true)
        tr_pred_c = np.concatenate(tr_pred)
        tr_f1 = f1_score(tr_true_c, tr_pred_c, average="macro")
...
try:
    ...
    if is_oom(e):
        ...
        print(f"\n⚠️  OOM detected at batch_size={current_batch}. "
              f"Retrying with batch_size={halved}.")
        import gc
        gc.collect()
        ...
        try:
            ...
            if is_oom(e2):
                print(f"\nERROR: OOM persists after halving "
                      f"(batch_size={current_batch}). Stopping.")
                raise SystemExit(1)
            else:
                raise
    else:
...
print(f"✅ [Step 6 — training done]")

print("\nLoading best checkpoint for test evaluation ...")
config = AutoConfig.from_pretrained(SAVE_DIR, num_labels=4)
config.classifier_dropout = CLASSIFIER_DROPOUT
...
hdr = "            " + " ".join(f"{n:>13s}" for n in LABEL_NAMES)
print(hdr)
...
assert len(test_df) == len(y_pred), "test row count mismatch"
out = pd.DataFrame({
...
print(f"\n✅ [Step 7 — saved test predictions to {PRED_CSV}]")
print(f"\n✅ TASK 4 complete.")
