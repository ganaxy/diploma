...
DATA_CSV   = os.path.join(WORK_DIR, "stage1_dataset.csv")
SAVE_DIR   = os.path.join(WORK_DIR, "best_stage1_model")
...
required = {"id", "text_normalized", "label_stage1", "split", "source"}
missing = required - set(df.columns)
if missing:
    print(f"ERROR: missing cols in stage1_dataset.csv: {missing}")
    raise SystemExit(1)

df["input_text"] = df.apply(
    lambda r: f"[{r['source']}] {str(r['text_normalized'] or '')}", axis=1
)
df["label_id"] = df["label_stage1"].map(STAGE1_MAP)
...
print("✅ [Step 2.1 — Stage 1 data loaded]")

tokenizer = AutoTokenizer.from_pretrained(HF_NAME)
print("✅ [Step 2.2 — tokenizer loaded]")
...
print("✅ [Step 2.3 — max_length/batch_size decided]")
...
print("✅ [Step 2.5 — class weights computed]")
...
def do_full_training(batch_size):
    print(f"\nTraining Stage 1 with batch_size={batch_size}, "
...
try:
    ...
    if is_oom(e):
        ...
        raise
print(f"\nStage 1 training complete. best_epoch={best_epoch} "
      f"best_val_macro_f1={best_val_f1:.4f}")
print("✅ [Step 2.6 — Stage 1 training done]")

print("\nLoading best Stage 1 checkpoint for test evaluation ...")
config = AutoConfig.from_pretrained(HF_NAME, num_labels=NUM_LABELS)
config.classifier_dropout = CLASSIFIER_DROPOUT
...
print("\n" + "═"*64)
print(" STEP 2 — Stage 1 test-set results (POSITIVE/NEUTRAL/NEGATIVE)")
print("═"*64)
...
print("            " + " ".join(f"{n:>12s}" for n in LABEL_NAMES))
for i, row in enumerate(cm):
    cells = " ".join(f"{v:>12d}" for v in row)
    print(f"  {LABEL_NAMES[i]:<10s}{cells}")

print("\n✅ STEP 2 (Stage 1) complete. "
      "STOP — waiting for confirmation to run STEP 3 (Stage 2 binary).")
