...
The server prints a local URL (typically `http://127.0.0.1:7860`).
...
- Models are loaded **once at startup** and cached in memory. Inference is
  ~20–35 ms on DirectML and ~200–300 ms on CPU (per the thesis §5).
- Safetensors are **referenced by absolute path**, not copied.
